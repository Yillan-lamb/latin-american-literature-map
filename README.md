...
当前版本为 `0.3.6`。阶段 1 已完成。阶段 2 的双作家来源候选池 R2 已通过：正式选择博尔赫斯《阿莱夫》《小径分岔的花园》《虚构集》和李斯佩克朵《活水》《星辰时刻》《家庭纽带》共六部作品，并完成 21 条候选来源的等级核定。下一步由外部 AI 先做八个合法开放来源的访问与页段定位，Codex 复核后再启动窄范围 L2；候选数据不会直接进入主数据库。
...
GitHub 是本项目公开版本历史的权威来源。通过审计的变更先在本地累积；仅在用户明确通知“上传”或“提交并上传”时，Codex 才对可公开的累积变更做批量提交和上传。
