...
当前版本为 `0.9.0`。USER 已于 2026-08-11 以 B/A/A/A 通过 N3；章程 1.4.0 将 V1 最低关系门槛调整为 75、把 150 作为 V1.1 扩展目标，Schema 0.3 新增 `BASED_ON_EVENT`（仅 `work → event`）。当前正式候选包包含 74 个来源、144 个规范实体、76 个可审核关系、40 个 hold、238 条事实素材和 40 张内容卡，并同时提供 CSV、JSON、SQLite 和 18 工作表 Excel。项目进入阶段 5，准备最终一致性审计与 N4 正式发布审核。
...
- `docs/阶段3_多作家研究汇总与缺口报告.md`：阶段 3 批次总量、覆盖、缺口和阶段 4 路由；
