...
当前版本为 `0.2.1`。阶段 0 的项目章程、研究与 OCR 规范、统一任务源、外部 AI 分工和交接模板已经完成，项目仍处于 **N1 范围基线审核**。首个外部任务包 `N1-OCR-001` R2 已通过 Codex 独立复检，并作为阶段 1 的先行交付证据登记；该登记不等于解锁阶段 1，也不分配正式来源 ID 或把候选写入暂存区。
