...
当前版本为 `0.3.0`。用户已经通过 **N1 范围基线审核**，项目正式进入阶段 1。首个外部任务包 `N1-OCR-001` R2 已通过 Codex 独立复检，六份资料已分配 `SRC-0001`~`SRC-0006`。当前由外部 AI 进行正式来源 ID 回填，随后由另一 AI 独立复核；复核通过前，候选数据不进入暂存区。
