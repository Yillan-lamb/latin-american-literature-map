...
最高治理文件：`PROJECT_CHARTER.md`。本任务源只记录动态状态，任何任务或外部交接不得改变总章程；来源级证据、效率规则、V1 节点式 GitHub 自动发布及 N3 关系门槛调整均已获用户明确授权，章程当前版本为 1.4.0。
...
- 负责人：`CODEX-PM`
- 产物：冻结总章程 `PROJECT_CHARTER.md`；阶段 0 支持文档 `docs/archive/v1-foundation/阶段0_项目章程.md`
- 验收：明确使命、V1 范围、成功标准、角色、节点和版权边界。

### ✅ V1-S0-002：建立研究、OCR、来源与数据规范

- 负责人：`CODEX-PM`
- 产物：`docs/archive/v1-foundation/阶段0_研究与数据规范.md`（历史规范；当前增量执行以《数据新增与版本维护操作手册》为准）
...
- 负责人：`CODEX-PM`
- 产物：`docs/archive/v1-foundation/N1_范围基线审核包.md`
- 验收：用户只需对范围、名单、主题、资料策略和分工做节点决定。

### ✅ V1-S0-006：建立 V1 节点式 GitHub 自动发布规则

- 负责人：`CODEX-PM`
- 用户授权：2026-08-10 明确批准修改 `PROJECT_CHARTER.md`。
...
- 输入：`docs/archive/v1-foundation/N1_范围基线审核包.md`
- 决定：批准、批准并提出调整、暂不批准。
- 通过后解锁：阶段 1 全部任务。
- 审核结果：用户于 2026-08-04 批准按建议范围执行；结论见 `docs/archive/v1-foundation/N1_审核结论.md`。
...
- 接管提示词：`work/external-ai/新外部AI_项目接管与首任务提示词.md`。
- 后续路线图：`work/external-ai/后续任务安排_新外部AI.md`。
...
- 有效启动提示词：`work/external-ai/新外部AI_项目接管与首任务提示词.md`。
...
- 审核包：`docs/archive/v1-foundation/N2_知识模型试点审核包.md`。
...
- 负责人：`CODEX-PM`
- 产物：`docs/archive/v1-foundation/阶段3_多作家研究汇总与缺口报告.md`
...
- 审核结果：USER 于 2026-08-11 回复 B/A/A/A；明确批准修改 `PROJECT_CHARTER.md` 的 V1 关系门槛，并批准 Schema 加法兼容升级。
- 结论文件：`docs/N3_审核结论.md`；实施后 Schema 0.3、关系 76/75，author→event 不建模，政治诗歌维持 3/1/1。
...
- 负责人：`CODEX-REVIEW`
- 完成结果：`docs/阶段5_最终一致性与引用审计.md`，结论 pass；四格式、外键、引用、Schema 和公开边界均通过。

### ✅ V1-S5-003：生成版本摘要和已知问题清单

- 负责人：`CODEX-PM`
- 产物：`docs/V1_版本摘要.md`、`docs/V1_已知问题.md`。
...
- 负责人：`USER`
- 依赖：阶段 5 全部任务
- 审核材料：`docs/V1_版本摘要.md`、`docs/V1_已知问题.md`、`docs/阶段5_最终一致性与引用审计.md`。
...
- 负责人：`CODEX-PM`
- 依赖：V1-N4-001
- 产物：`docs/N4_正式发布结论.md`、`v1.0.0` 标签和 GitHub Release。
...
- 产物：`docs/数据新增与版本维护操作手册.md`、`docs/V1_工作流程审计摘要.md`、`docs/旧规则冲突与淘汰清单.md`、`data/master/`、`scripts/validate_master.py`、`scripts/apply_migration.py`、`scripts/export_from_sqlite.py`。
