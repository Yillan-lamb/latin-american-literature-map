...
写 `V1_MASTER.sqlite`；分配 `V1-ENT/V1-FCT/V1-REL/SRC-` 正式 ID（只用 CAND-D-* 临时 ID；作者实体只引用既有 V1 正式 ID）；批准自己的输出；修改 `PROJECT_CHARTER.md`/`TASKS.md`/`V2_TASKS.md`/`CHANGELOG.md`；git 操作；改 `site/`；研究 packet 外对象。
