# Research Schema 与迁移规则

- 版本：`0.4`
- 性质：Research Data 的稳定、加法兼容数据契约
- 负责人：`CODEX-DATA`
- 生效范围：研究、候选、暂存、数据库与导出流程
- 演进记录：历史加法升级由 `CHANGELOG.md`、正式决策和迁移记录追踪

## 1. 目的

本文件规定研究数据采用的实体分层、关系词、证据规则和迁移口径。`BASED_ON_EVENT` 与 `APPEARS_IN` 均为已经批准的加法兼容规则，既有关系与数据不因规则升级而被重写。外部任务不得自行增加实体层或关系词；后续如需改变，仍须由 Codex 形成兼容性提案并提交相应用户节点决定。
