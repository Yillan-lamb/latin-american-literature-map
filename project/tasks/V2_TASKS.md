...
- **🔒 WCD-07 Major Works Research Expansion（核心作家重要作品补全）**：只处理 WCD-04 已列入优先级、且 exact-title / current-author precheck 未发现 current-master match 的 major-work research candidates。WCD-07 不继承“P0/P1 = 已批准新增”的错误含义，也不得把该预检解释为语义无重复；每一项仍须走 `duplicate check → source verification → entity/collection overlap check → Chinese edition check（辅助）→ candidate changeset → independent reviewer → SQLite migration`。
