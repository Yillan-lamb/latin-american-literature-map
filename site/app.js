...
function renderPlace(id) {
  ...
  app.innerHTML = `<section class="page-header ${fictional ? "fictional-header" : ""}"><p class="eyebrow">${fictional ? "文学虚构空间" : "文学地点"}</p><h1 class="display-title">${escapeHtml(item.name_zh)}</h1><div class="page-header-meta"><span class="tag ${fictional ? "blue" : "green"}">${fictional ? "由作品创造的空间" : "现实地点"}</span>${item.original_name ? `<span class="tag">${escapeHtml(item.original_name)}</span>` : ""}</div><p class="lede">${escapeHtml(text)}</p></section><section class="content-grid"><div class="content-copy">${copy.spatial_meaning ? `<h2>这里为什么值得注意</h2><p>${escapeHtml(copy.spatial_meaning)}</p>` : ""}<h2>${fictional ? "它出现在哪里" : "这里为什么与文学有关"}</h2><div class="linked-list">${[...works, ...authors].map((relation) => { const source = entity(relation.source_entity_id); return `<a href="${hrefFor(routeTypeFor(source.entity_id), source.entity_id)}"><strong>${escapeHtml(source.name_zh)}</strong><span>${escapeHtml(relation.description_zh || relationLabel(relation.relation_type))}</span></a>`; }).join("") || "<p>相关文学资料仍在补充。</p>"}</div>${item.entity_id ? researchPanel(item.entity_id, sourceIds) : sourceList(sourceIds)}</div><aside class="side-rail">${copy.exploration_route ? `<div class="info-box"><h3>从这里继续</h3><p>${escapeHtml(copy.exploration_route)}</p></div>` : ""}<div class="info-box"><h3>直接入口</h3>${works.map((relation) => `<a class="text-link block" href="${hrefFor("work", relation.source_entity_id)}">${escapeHtml(displayName(relation.source_entity_id))} →</a>`).join("")}${authors.map((relation) => `<a class="text-link block" href="${hrefFor("author", relation.source_entity_id)}">${escapeHtml(displayName(relation.source_entity_id))} →</a>`).join("")}</div>${fictional ? `<div class="info-box"><h3>空间说明</h3><p>文学虚构空间不使用现实坐标。地图将它作为独立的文学入口呈现。</p></div>` : ""}</aside></section>`;
}
...
function renderAuthor(id) {
  ...
  ${copy.literary_features ? `<section class="section"><div class="section-heading"><h2>写作的辨识度</h2><p>从来源可支撑的叙事与语言特征进入。</p></div><div class="feature-grid">${copy.literary_features.map((feature) => `<article><h3>${escapeHtml(feature.title)}</h3><p>${escapeHtml(feature.text)}</p></article>`).join("")}</div></section>` : ""}
...
function renderWork(id) {
  ...
  ${copy.narrative_features ? `<section class="section"><div class="section-heading"><h2>叙事与形式</h2><p>这些特征均有对应资料支撑。</p></div><div class="feature-grid">${copy.narrative_features.map((feature) => `<article><h3>${escapeHtml(feature.title)}</h3><p>${escapeHtml(feature.text)}</p></article>`).join("")}</div></section>` : ""}
