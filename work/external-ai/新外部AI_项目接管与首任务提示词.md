...
2. 完整读取以下文件，不依赖用户聊天记录：
   - `PROJECT_CHARTER.md`：最高冻结章程，禁止修改、移动、重命名或删除；
   - `TASKS.md`：唯一动态任务状态源，只读；
   - `拉丁美洲文学地图_项目决策记录.md`；
   - `docs/阶段0_研究与数据规范.md`；
   - `docs/外部AI任务分工与交接手册.md`；
   - `docs/外部AI执行工作流与自检手册.md`；
   - `work/external-ai/后续任务安排_新外部AI.md`；
   - 当前任务卡及任务卡列出的全部输入。
...
- 不修改 `PROJECT_CHARTER.md`、`TASKS.md`、正式来源登记表、项目决策记录、CHANGELOG 或任何 Git/GitHub 状态。
...
后续任务必须等 Codex验收并在 `TASKS.md` 解锁后再执行，不得自行从路线图领取。
